  ...
  */
  ...
  function getHashtagRanges(blockText, hashtagConfig) {
    ...
    if (hashtagConfig) {
      ...
      for (; _text.length > 0 && startIndex >= 0;) {
        if (_text[0] === trigger) {
          startIndex = 0;
          counter = 0;
          _text = _text.substr(trigger.length);
        } else {
          startIndex = _text.indexOf(separator + trigger);

          if (startIndex >= 0) {
            _text = _text.substr(startIndex + (separator + trigger).length);
            counter += startIndex + separator.length;
          }
        }

        if (startIndex >= 0) {
          var endIndex = _text.indexOf(separator) >= 0 ? _text.indexOf(separator) : _text.length;

          var hashtag = _text.substr(0, endIndex);
  ...
  */
  ...
  function getSectionMarkup(block, entityMap, section, customEntityTransform) {
    ...
    if (section.type === 'ENTITY') {
      ...
      sectionText = customEntityTransform(null, text);
    }
